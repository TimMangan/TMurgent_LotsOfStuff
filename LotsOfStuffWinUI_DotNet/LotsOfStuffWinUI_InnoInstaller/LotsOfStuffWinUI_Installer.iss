...
#define Redistributable "..\\Redistributable\\WindowsAppRuntimeInstall-x64.exe"
...
Name: winappruntime; Description: "Install the Windows App Runtime 2.4.0 x64 (required for the app to run)"; GroupDescription: "Additional tasks:"; Flags: unchecked
...
begin
  ...
  // If the user did not select the optional Windows App Runtime task, skip installation
  if not IsTaskSelected('winappruntime') then
	Exit;
  ...
  if MsgBox('Windows App Runtime installation failed (exit code: ' + IntToStr(ResultCode) + ').' + #13#10 +
			'Do you want to abort installation of the application?', mbError, MB_YESNO) = IDYES then
  begin
	Result := False;
  end
  else
